	...
	<script type="text/javascript">
			...
			app.controller('authoriCon',['$scope','$http',function($scope, $http){	
				...
				$scope.ckRowClick = function(index){
					...
					$scope.temDataEmp({"empID":$scope.emps[index].empID});
					
					//alert($scope.emps[index].empID);
				}

				$scope.temDataEmp = function(empId){
					$scope.temEmp = [];	
					$scope.temEmp.push(angular.copy($scope.temEmp[empId]));
					dis($scope.temEmp);
				}
				...
				$scope.empIndexSet = function(bool,custId){
					var n = $scope.searchIndexByEmpID(custId);
					if(n != false || n==0){
				...
				$scope.searchIndexByEmpID = function(empId){		
				...
				function clkCustomer(obj){
					...
					var empId = obj.getAttribute("value");
					var bool = false;
					if ($(obj).is(':checked')) {
						bool = true;
					}
					angular.element(document.getElementById('authoriCon')).scope().empIndexSet(bool,empId);	
				}
