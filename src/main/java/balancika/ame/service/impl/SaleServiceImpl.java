...
import balancika.ame.entities.tansaction.SaleDetail;
...
public class SaleServiceImpl implements SaleService{
	...
	@Override
	public Sale getSale(Sale sale, MeDataSource dataSource) {		
		try (Connection con = DBConnection.getConnection(dataSource)){			
			...
			while(rs.next()){ i++;			
				...
				saleDetail.setSaleId(rs.getString("SalID"));
				...
				saleDetail.setQty(rs.getDouble("SalQty"));
			...
			sales.setSaleDetails(saleDetails);
			return sales;
		}catch (Exception e) {
			e.printStackTrace();
		}				
